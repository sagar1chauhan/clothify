...
const ReturnRequestDetail = () => {
  ...
  useEffect(() => {
    ...
    // Use the same filtering logic as the list page for consistency and robustness
    const vendorRequests = requests.filter((request) => {
      if (request.items && Array.isArray(request.items)) {
        return request.items.some((item) => {
          // Check if product belongs to vendor directly
          if (item.vendorId === vendorId) return true;

          // Fallback: Check if the order it belongs to has vendor items for this vendor
